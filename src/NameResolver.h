...
private:
	void clearSymbolDepth(); // remove all symb with depth > this->depth
	bool isNameValid(const std::string& name, bool type = false);
	AstDecl* findDecl(const std::string& name, bool type = false);
